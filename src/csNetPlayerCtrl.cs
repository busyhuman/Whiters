...
public class csNetPlayerCtrl : NetworkBehaviour {
	/* 위에서부터
	 * 플레이어 상태목록
	...
	void Start(){
		...
		if (isLocalPlayer)
			csCamera.targetTr = this.transform;
	}

	void OnEnable(){
		gameObject.tag = "Player";
		gameObject.GetComponent<csNetPlayerCtrl> ().enabled = true;
	}
	...
	[Command]
	void CmdChangeDoISetActive(int _doIsetActive){
		doISetActive = _doIsetActive;
	}
	...
	void FixedUpdate(){
		...
		Debug.Log ("networkSer" + NetworkServer.active);
		Debug.Log ("networkCli" + NetworkClient.active);
		*/
		...
		// 서버에서 클라이언트가 잘 안됨.
		if (!isServer) {
			CmdChangeDoISetActive (doISetActive);
		}
